...
/** 21st: aghasisahakyan1/star-button — Vidhara teal/lavender accents */
export function StarButton({
  ...
  return (
    <button
      ...
      type={type}
      className={cn(
        'group/star-button relative z-[3] inline-flex h-9 items-center justify-center gap-2 overflow-hidden rounded-3xl',
        'border border-border bg-[#f3efe8] px-3.5 py-1.5 text-xs font-medium whitespace-nowrap transition-colors',
        'dark:bg-[#2a2a2a]',
        'disabled:pointer-events-none disabled:opacity-50',
        active &&
          'border-teal ring-2 ring-teal/50 ring-offset-2 ring-offset-surface',
        className,
      )}
      {...props}
    >
      ...
      />
      <div
        className="absolute inset-0 z-[4] overflow-hidden rounded-[inherit] border-plum/15 text-lavender dark:border-lavender/20 dark:text-teal"
        style={{ borderWidth: 'var(--border-width)' }}
        aria-hidden="true"
      >
        <StarBackground color={backgroundColor} clipId={clipId} />
      </div>
      <span className="relative z-10 inline-block bg-gradient-to-t from-[#45355e] to-[#5ba8a8] bg-clip-text text-transparent dark:from-[#f3efe8] dark:to-[#8b7ba8]">
        {children}
      </span>
    </button>
  )
